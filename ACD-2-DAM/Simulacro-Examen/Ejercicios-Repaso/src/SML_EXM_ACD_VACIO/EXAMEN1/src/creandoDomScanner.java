...
public class creandoDomScanner {
    /*EJ 2- CREAR UN DOCUMENTO XML CON DOM INTRODUCIENDO LOS DATOS CON TECLADOS*/
    ...
    public static void escribirXML(String archivo) {
        try {
            ...
            System.out.println("¡!A CONTINUACIÓN INTRODUCE LOS REGISTROS EN EL SIGUIENTE ORDEN: " 
                            + "\n" + "1. ID (NÚMERO ENTERO)" + "\n" +  "2. NOMBRE (CADENA DE CARACTERES CON ESPACIOS)" + "\n" + "3. SALARIO (NÚMERO DECIMAL)" + "\n"
                            + "\n" + "¿TECLEA LA CANTIDAD DE EMPLEADOS QUE QUIERES AGREGAR POR FAVOR!");
                    int numeroEmpleados = entrada.nextInt();
                    System.out.println();
                for(int i = 0; i < numeroEmpleados; i++) {
                    System.out.println("1. INTRODUCE NÚMERO DEL ID DEL EMPLEADO: " + "\n");
                    int idEmpleado = entrada.nextInt();
                    entrada.nextLine();
                    System.out.println();
    
                    System.out.println("2. INTRODUCE EL NOMBRE DEL EMPLEADO: " + "\n");
                    String nombreEmpleado = entrada.nextLine();
                    System.out.println();
    
                    System.out.println("3. INTRODUCE SALARIO DEL EMPLEADO: " + "\n");
                    double salarioEmpleado = entrada.nextDouble();
                    System.out.println();
    
                    empleados.add(new Empleado(idEmpleado, nombreEmpleado, salarioEmpleado));
                }
            entrada.close();
